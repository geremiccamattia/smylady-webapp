...
export function SpotifyTrackPreview({ track, compact = false }: SpotifyTrackPreviewProps) {
  return (
    <a
      ...
      className={`flex items-center gap-3 rounded-lg border bg-card hover:bg-muted/50 transition-colors ${
        compact ? 'p-2' : 'p-3'
      }`}
    ...
    >
      {track.albumCover ? (
        <img
          ...
          className={`rounded object-cover flex-shrink-0 ${compact ? 'h-10 w-10' : 'h-14 w-14'}`}
        />
      ) : (
        <div className={`rounded bg-[#1DB954] flex items-center justify-center flex-shrink-0 ${compact ? 'h-10 w-10' : 'h-14 w-14'}`}>
      ...
      <div className="flex items-center gap-1 text-[#1DB954] flex-shrink-0">
        <svg className={compact ? 'h-4 w-4' : 'h-5 w-5'} viewBox="0 0 24 24" fill="currentColor">
